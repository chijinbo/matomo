...
class PasswordResetter
{
    ...
    public function initiatePasswordResetProcess($loginOrEmail, $newPassword)
    {
        ...
        $this->savePasswordResetInfo($login, $newPassword, $resetToken);
    ...
    public function checkValidConfirmPasswordToken($login, $resetToken)
    {
        ...
        if ($resetInfo === false
            ...
            || !$this->isTokenValid($resetToken, $user, $resetInfo['resetToken'])
        ) {
     ...
     * @param string $resetToken The reset token in db
     * @return bool true if valid, false otherwise.
     */
    public function isTokenValid($token, $user, $resetToken)
    {
        $now = time();
        ...
        if($token === $resetToken){
            for ($i = 0; $i <= 24; $i++) {
                $expiry = strftime('%Y%m%d%H', $now + $i * 60 * 60);
                if ($this->passwordHelper->verify($expiry . $user['login'] . $user['email'] . $user['ts_password_modified'] . $user['password'],$resetToken)) {
                    return true;
                }
            }
        }
    ...
    {
        /*
         * The generated password reset token will be saved in option table
         *
         */
        if (!$expiryTimestamp) {
        ...
        $token =  $this->passwordHelper->hash($expiry . $user['login'] . $user['email'] . $user['ts_password_modified'] . $user['password']);
        return $token;
    }
     ...
     */
    private function savePasswordResetInfo($login, $newPassword, $resetToken)
    {
